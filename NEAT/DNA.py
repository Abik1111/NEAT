...
class Genome:
    ...
    def compute_fitness(self, result):
        ...
        # print("ip_keys=",self.input_keys)
        # print("op_keys=",self.output_keys)
        # print(total)
        if total < 1e-3:
            total = 1e-3
    ...
    def copy(self):
        ...
        cop.inputs=self.inputs.copy() 
        cop.outputs=self.outputs.copy() 
        cop.hidden_keys=self.hidden_keys.copy() 
        cop.input_keys=self.input_keys.copy()
        cop.output_keys =self.output_keys.copy()
        cop.innov_num=self.innov_num 
        cop.num_nodes=self.num_nodes 
        cop.node_gene=self.node_gene.copy() 
        cop.connection_gene=self.connection_gene.copy() 
        cop.fitness=self.fitness 
        cop.distance=self.distance.copy() 
        return cop

    # def update_node_gene(self,inputs,outputs):
    ...
    def predict(self,inputs):
        self.inputs=inputs
        for a,b in zip(self.input_keys,self.inputs):
            self.node_gene[a]=b
        self.propagate()
        dir_keys = sorted([(x, y) for y, x in self.distance.items()])
        sortd_dict = dict([(x, y) for (y, x) in dir_keys])
